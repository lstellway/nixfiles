...
- Go-language idioms (concurrency, error wrapping, context patterns, testing setup) → a Go-language specialist.
- Repository pattern / DAL architecture / where to draw the boundary between domain and persistence → a software-architecture specialist.
- "Should this constraint live in the DB or the app?" / "What consistency model do we need?" → a data-integrity specialist.
- "How slow is too slow?" / "Where should we spend our optimization budget?" / engine-side index design → a performance-engineering specialist.
- Threat modeling / secret storage / "is this secure?" → a security specialist.
- Engine-specific features (Postgres `JSONB` operators, MySQL replication, query planner specifics) → engine docs directly (no dedicated engine specialist exists yet).
